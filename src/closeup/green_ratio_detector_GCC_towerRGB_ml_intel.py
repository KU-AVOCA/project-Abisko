...
def quantify_vegetation_kmeans(img):
    """
    Quantifies green vegetation using K-means clustering with Intel acceleration.
    ...
    Returns:
        tuple: Overall green metrics, green mask, class1 metrics, class2 metrics, visualization
    """
    try:
        ...
        if np.sum(non_zero_mask) > 0:  # Check if there are green pixels
            ...
            try:
                # Apply Intel-accelerated K-means clustering (K=2 for trees and understory)
                with config_context(target_offload="auto"):
                    # Use Intel-accelerated KMeans implementation from sklearnex
                ...
                print(f"Intel-accelerated KMeans failed: {e}, falling back to OpenCV implementation")
                
                # Fallback to OpenCV KMeans
