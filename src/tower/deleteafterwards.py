...
def process_single_thermal_image(csv_path, input_root, output_root):
    """
    Process a single thermal CSV file and save as .mat file.
    ...
    Returns:
    dict: Processing status for the image
    """
    ...
    try:
        ...
        mat_path = os.path.join(output_root, 'matimages', imposition + '_' + os.path.basename(rel_path).strip('.csv') + '.mat')
        os.makedirs(os.path.dirname(mat_path), exist_ok=True)
        
        # Save figure as MATLAB .mat file
        savemat(mat_path, {'thermal_image': thermal_data})
...
def process_thermal_images_parallel(input_root, output_root, max_workers=None):
    """
    Process thermal CSV files in parallel and save as .mat files.
    ...
    os.makedirs(os.path.join(output_root, 'matimages'), exist_ok=True)
    ...
    print(f".mat files saved to: {os.path.join(output_root, 'matimages')}")

def main():
    """Main function to get user input and run the processing."""
    print("Thermal Image .mat File Generator (Parallel)")
    print("=" * 45)
    ...
    output_root = input("Enter the output root path (for .mat files): ").strip()
